defmodule PocketUrlWeb.LandingLive do
  ...
  def mount(_params, _session, socket) do
    {:ok, socket |> assign(:url, nil) |> assign(:base_url, @base_url)}
  end

  @impl true
  def handle_info({:url_shortened, url}, socket) do
  ...
  @impl true
  def handle_event("copy-short-url", _unsigned_params, socket) do
    Process.send_after(self(), :clear_flash, 5000)

    {:noreply,
     socket
     |> push_event("copy-to-clipboard", %{selector: "#short-url"})
     |> put_flash(:info, "Succesfully copied Short Url")}
  end
  ...
  def render(assigns) do
    ~H"""
    <div
      id="short-url-container"
      class="flex flex-wrap justify-center pb-16"
      phx-hook="CopyToClipboard"
    >
      <div class="flex justify-center w-full">
        <img src={~p"/images/logo.webp"} width="350" />
      </div>

      <div class="w-full text-center">
        <p>Shorten your URLs! Completely secure, completely safe, and completely free!</p>
      </div>

      <div class="w-full flex flex-wrap justify-center mt-4 max-w-5xl mx-auto">
        <div :if={@url} class="w-full flex flex-col mt-4">
          <div class="">
            <span>Your original url</span>
          ...
          <div class="flex flex-col mt-8">
            ...
            <div class="flex items-center">
              <input
                id="short-url"
                class="block mr-4 w-[300px]"
                type="text"
                readonly
                value={"#{@base_url}/#{@url.short_code}"}
              />
              <button type="button" id="copy-url" phx-click="copy-short-url">
                <svg
        ...
        <.live_component module={PocketUrlWeb.Forms.Url} id="url-shortener-form" />
      </div>
    </div>
    """
  end
